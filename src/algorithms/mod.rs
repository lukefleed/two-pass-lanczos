...
/// use lanczos_project::algorithms::lanczos_scratch;
///
/// let a = Mat::<f64>::identity(4, 4);
/// let req = lanczos_scratch(&a.as_ref(), faer::Par::Seq);
/// let mut mem = MemBuffer::new(req);
/// let _stack = MemStack::new(&mut mem);
/// ```
...
mod tests {
    ...
    fn test_k_zero_returns_empty_decomposition() -> Result<()> {
        ...
        let result = lanczos_standard(&a, b.as_ref(), 0, Par::Seq, Reorthogonalization::None, stack, None)?;
    ...
    fn test_k_one_produces_single_step() -> Result<()> {
        ...
        let result = lanczos_standard(&a.as_ref(), b.as_ref(), 1, Par::Seq, Reorthogonalization::None, stack, None)?;
    ...
    fn test_breakdown_scenario() {
        ...
        let result = lanczos_standard(&a.as_ref(), b.as_ref(), k, Par::Seq, Reorthogonalization::None, stack, None).unwrap();
        assert_eq!(result.decomposition.steps_taken, 1);
    }
    ...
    fn test_zero_vector_input_returns_error() {
        ...
        let mut mem = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let stack = MemStack::new(&mut mem);
        assert!(lanczos_standard(&a, b.as_ref(), 2, Par::Seq, Reorthogonalization::None, stack, None).is_err());
    }
    ...
    fn run_decomposition_consistency_test_for_instance(instance: &TestInstance) -> Result<()> {
        ...
        let standard_output = lanczos_standard(&a.as_ref(), b.as_ref(), k, Par::Seq, Reorthogonalization::None, stack, None)?;
    ...
    fn run_lanczos_relation_test_for_instance(instance: &TestInstance) -> Result<()> {
        ...
        let result_k = lanczos_standard(&a.as_ref(), b.as_ref(), k, Par::Seq, Reorthogonalization::None, stack, None)?;
        let result_k_plus_1 = lanczos_standard(&a.as_ref(), b.as_ref(), k + 1, Par::Seq, Reorthogonalization::None, stack, None)?;
    ...
    #[test]
    fn test_full_reorthogonalization_maintains_orthogonality() -> Result<()> {
        use super::Reorthogonalization;

        let n = 50;
        let k = 40;
        let mut a = Mat::<f64>::zeros(n, n);
        for i in 0..n {
            a[(i, i)] = 1.0 + (i as f64) * 1e6 / (n as f64);
        }
        let b = Mat::from_fn(n, 1, |i, _| (i + 1) as f64);
        let mut mem = MemBuffer::new(a.as_ref().apply_scratch(1, Par::Seq));

        let stack = MemStack::new(&mut mem);
        let result_full = lanczos_standard(
            &a.as_ref(),
            b.as_ref(),
            k,
            Par::Seq,
        ...
        let v = result_full.v_k.as_ref();
        let s = v.ncols();
        let ortho_err =
            (&Mat::<f64>::identity(s, s) - v.adjoint() * v).norm_l2();
        ensure!(
            ortho_err < 1e-12,
            "Full reorthog error too large: {ortho_err}"
        );
        Ok(())
    }
}
