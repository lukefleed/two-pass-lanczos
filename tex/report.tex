...
\end{equation}
where $\mathbf{A} \in \mathbb{C}^{n \times n}$ is a large, sparse Hermitian matrix and $f: \Omega \subseteq \mathbb{C} \to \mathbb{C}$ is a function defined on a set $\Omega$ containing the spectrum of $\mathbf{A}$, $\sigma(\mathbf{A})$. A formal definition of a matrix function can be established in several equivalent ways, including through the Jordan canonical form, a Cauchy integral representation, or a Hermite interpolating polynomial \cite{golub2013matrix, frommer2008matrix}.

The most direct definition arises when $f$ is a polynomial. If $p(z) = \sum_{j=0}^d c_j z^j$, the matrix function $p(\mathbf{A})$ is defined by direct substitution, $p(\mathbf{A}) = \sum_{j=0}^d c_j \mathbf{A}^j$. For a general function $f$ that is analytic on $\Omega$, a key result from the theory of matrix functions is its equivalence to a polynomial in $\mathbf{A}$.
...
This polynomial equivalence is the theoretical foundation for the methods we consider. It implies that the exact solution vector $\mathbf{x} = f(\mathbf{A})\mathbf{b}$ is itself an element of a Krylov subspace. Since $\mathbf{x} = p(\mathbf{A})\mathbf{b}$, the solution lies in $\mathcal{K}_{d+1}(\mathbf{A}, \mathbf{b})$, where $d$ is the degree of the interpolating polynomial $p$.

The explicit computation of the matrix $f(\mathbf{A})$ is generally infeasible due to its high computational cost and the fact that $f(\mathbf{A})$ is typically a dense matrix even when $\mathbf{A}$ is sparse \cite{frommer2008matrix}. Our focus, therefore, is on methods that compute the action $\mathbf{x} = f(\mathbf{A})\mathbf{b}$ directly. This problem is fundamental in many areas of scientific computing \cite{golub2013matrix}. A primary example is the solution of a large system of linear equations $\mathbf{Ax} = \mathbf{b}$, which corresponds to the case where $f(z) = z^{-1}$ and $\mathbf{A}$ is nonsingular \cite{saad2003iterative}. Another critical application arises in the numerical solution of systems of linear ordinary differential equations. The solution to the initial value problem $\mathbf{y}'(t) = \mathbf{A}\mathbf{y}(t)$, with $\mathbf{y}(0) = \mathbf{y}_0$, is given by $\mathbf{y}(t) = \exp(t\mathbf{A})\mathbf{y}_0$. This computation is an instance of our problem where the function is the matrix exponential, $f(z) = \exp(tz)$ \cite{frommer2008matrix}.

\subsection{Krylov Subspace Methods: The Standard Approach}
The connection between matrix functions and polynomial approximation provides a direct path to the use of Krylov subspace methods \cite{frommer2008matrix, saad2003iterative}. Since the exact solution lies in a Krylov subspace, it is natural to seek an approximation within a subspace of a much smaller, computationally manageable dimension.

\begin{definition}[Krylov Subspace]
    Given a matrix $\mathbf{A} \in \mathbb{C}^{n \times n}$ and a vector $\mathbf{b} \in \mathbb{C}^n$, the $k$-th Krylov subspace generated by $\mathbf{A}$ and $\mathbf{b}$ is the vector space spanned by the first $k$ powers of $\mathbf{A}$ applied to $\mathbf{b}$:
    \begin{equation}
        \mathcal{K}_k(\mathbf{A}, \mathbf{b}) = \text{span}\{\mathbf{b}, \mathbf{Ab}, \dots, \mathbf{A}^{k-1}\mathbf{b}\}.
    \end{equation}
\end{definition}

The Lanczos algorithm constructs an approximation $\mathbf{x}_k \in \mathcal{K}_k(\mathbf{A}, \mathbf{b})$ that is optimal in a specific sense related to polynomial approximation. The quality of this approximation is rigorously linked to the error of the best uniform polynomial approximant of $f$ on the spectrum of $\mathbf{A}$. For a Hermitian matrix, this relationship is formalized by the following error bound.
...
This bound provides the theoretical justification for the method's efficacy. It demonstrates that the convergence of the Lanczos approximation depends directly on how well the function $f$ can be approximated by a low-degree polynomial over the spectral interval of $\mathbf{A}$.

The construction of the approximation $\mathbf{x}_k$ is achieved through a projection process that can be summarized in three stages \cite{frommer2008matrix}. First, an orthonormal basis for the subspace $\mathcal{K}_k(\mathbf{A}, \mathbf{b})$ is constructed via the Lanczos iteration. Let this basis form the columns of the matrix $\mathbf{V}_k \in \mathbb{C}^{n \times k}$. Second, the high-dimensional operator $\mathbf{A}$ is projected onto this subspace, yielding a small $k \times k$ real symmetric tridiagonal matrix $\mathbf{T}_k = \mathbf{V}_k^H \mathbf{A} \mathbf{V}_k$. The original problem is thereby reduced to the evaluation of $f(\mathbf{T}_k)$, a task which is computationally feasible for small $k$.

Finally, the solution to the projected problem is lifted back to the original $n$-dimensional space to form the final approximation. Assuming the starting vector for the basis construction is $\mathbf{v}_1 = \mathbf{b} / \|\mathbf{b}\|_2$, the approximation is given by
\begin{equation}
    \mathbf{x}_k = \mathbf{V}_k f(\mathbf{T}_k) \mathbf{e}_1 \|\mathbf{b}\|_2,
\end{equation}
where $\mathbf{e}_1$ is the first canonical basis vector in $\mathbb{C}^k$ \cite[Eq. 32]{frommer2008matrix}.
...
% ===================================================================
In this section, we present a series of numerical experiments designed to validate the theoretical analysis and to assess the practical performance of the two-pass Lanczos algorithm. Our investigation is structured into three distinct experiments. The first empirically validates the memory-versus-computation trade-off as a function of iteration count. The second assesses the scalability of both algorithms with respect to the problem dimension. The third provides a comprehensive analysis of numerical stability and accuracy across several challenging problem scenarios.
