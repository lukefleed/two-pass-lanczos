...
\begin{definition}[Krylov Subspace]
    ...
    \end{equation}
\end{definition}
The fundamental principle of a Krylov subspace method is to seek an approximate solution $\mathbf{x}_k$ within the low-dimensional subspace $\mathcal{K}_k(\mathbf{A}, \mathbf{b})$, where $k \ll n$. This is achieved through a projection process that can be summarized in three distinct stages, as described in \cite{frommer2008matrix}. First, we construct an orthonormal basis, typically via the Arnoldi or Lanczos iteration, for the subspace $\mathcal{K}_k(\mathbf{A}, \mathbf{b})$. Let this basis be the columns of the matrix $\mathbf{V}_k \in \mathbb{C}^{n \times k}$.
...
\end{equation}
The coefficient $\beta_j$ is determined by the normalization condition $\|\mathbf{v}_{j+1}\|_2 = 1$, which gives $\beta_j = \|\mathbf{A}\mathbf{v}_j - \alpha_j \mathbf{v}_j - \beta_{j-1} \mathbf{v}_{j-1}\|_2$. This process is defined as the Lanczos iteration \cite{golub2013matrix}.
...
\begin{equation}
    ...
    \end{pmatrix}
\end{equation}
This is the fundamental relation of the Lanczos process \cite{golub2013matrix}. From the orthonormality of the columns of $\mathbf{V}_k$, i.e., $\mathbf{V}_k^H \mathbf{V}_k = \mathbf{I}_k$, it follows that the tridiagonal matrix $\mathbf{T}_k$ is the orthogonal projection of $\mathbf{A}$ onto the Krylov subspace $\mathcal{K}_k(\mathbf{A}, \mathbf{v}_1)$, since
\begin{equation}
...
In this section, we provide a formal description of the two distinct phases of the algorithm: an initial pass to generate the coefficients of $\mathbf{T}_k$ without storing the basis, and a second pass to reconstruct the solution vector. We then present a formal analysis of the method's core trade-off, quantifying its memory savings against its increased computational cost. Finally, we discuss the generality of this approach in comparison to alternative methods, such as those that employ function-specific short-term recurrences for the solution vector \cite{saad2003iterative}.
...
\subsection{Analysis of Computational and Memory Complexity}
...
\begin{proof}
    ...
    The two-pass method, by its design, executes two separate passes. The first pass requires $k$ matrix-vector products to generate the coefficients of $\mathbf{T}_k$. To synthesize the solution, the second pass must re-generate the Lanczos basis vectors, which compels us to re-compute the same sequence of $k$ matrix-vector products. This brings the total number of matrix-vector products to $2k$. The doubling of the computational work is a defining characteristic of this two-stage approach \cite{frommer2008matrix}.
\end{proof}
...
The accuracy of a computed solution $x_k$ is measured by its relative error with respect to a known ground-truth solution $x_{\text{true}}$. The stability of the basis generation is assessed by quantifying the loss of orthogonality of the Lanczos basis $V_k$ via the Frobenius norm $\|I - V_k^H V_k\|_F$, a known phenomenon in finite-precision implementations of the process \cite{golub2013matrix}. For the stability experiment, we also measure the numerical deviation, or \emph{drift}, between the standard basis $V_k$ and the re-generated basis $V'_k$, as well as the drift in the scalar recurrence coefficients. The ultimate impact is evaluated by the relative difference between the final solution vectors obtained from the one-pass and two-pass methods.
...
To permit the exact computation of solution error, we adopt a known-solution methodology, a standard validation technique in numerical analysis \cite{saad2003iterative}. We first define a ground-truth solution vector $x_{\text{true}}$ and then construct the right-hand side vector as $b := A x_{\text{true}}$.
...
As we reduce the problem dimension, this dynamic changes. For the medium-scale instance with 50k arcs, the results in Figure \ref{fig:medium_scale} (right panel) show that the two-pass method is consistently faster than the standard one-pass method. This performance reversal stems from the memory access patterns of the algorithms. The standard algorithm's reconstruction step accesses a large contiguous block of memory for $\mathbf{V}_k$, generating an inefficient access pattern with poor data locality that leads to a high rate of cache misses. The two-pass algorithm, by contrast, operates on a small working set of vectors at each step. The processor's cache hierarchy manages this small working set effectively, resulting in a high cache hit rate and superior performance.

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{assets/exp1/arcs50k_rho3_memory.pdf}
        \centerline{(a) Peak Memory Usage}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{assets/exp1/arcs50k_rho3_time.pdf}
        \centerline{(b) Wall-Clock Time}
    \end{minipage}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{assets/exp1/arcs5k_rho3_memory.pdf}
        \centerline{(a) Peak Memory Usage}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{assets/exp1/arcs5k_rho3_time.pdf}
        \centerline{(b) Wall-Clock Time}
    \end{minipage}
...
The second experiment assessed how the resource requirements of both algorithms scale with the problem dimension $n$. For this test, the number of iterations $k$ was fixed to a constant value, and a family of test matrices of increasing dimension $n$ was generated. The network topologies were constructed to maintain a constant ratio of arcs to nodes, ensuring comparable sparsity across all instances. For each problem size, both algorithms were executed, and their peak RSS and total wall-clock time were recorded. We expected the results to show that the memory advantage of the two-pass algorithm becomes increasingly significant as $n$ grows. Specifically, a plot of peak RSS versus $n$ should show a substantially steeper slope for the one-pass method than for the two-pass method. The ratio of the execution times was expected to remain close to two, confirming the consistent nature of the computational trade-off across different problem scales.

\subsection{Experiment 3: Numerical Stability}
The final experiment provides a formal investigation into the numerical stability of the two-pass approach, with the objective of quantifying the numerical error introduced by the re-generation of the Lanczos basis. For this analysis, a problem of moderate dimension was selected, for which the explicit storage of the basis matrices is feasible. Let $\mathbf{V}_k = [\mathbf{v}_1, \dots, \mathbf{v}_k]$ denote the basis generated by the standard one-pass algorithm, and let $\mathbf{V}'_k = [\mathbf{v}'_1, \dots, \mathbf{v}'_k]$ be the basis re-generated during the second pass using the stored coefficients $\{\alpha_j, \beta_j\}$. The analysis comprised three principal measurements performed as a function of the iteration count $k$. First, we computed the loss of orthogonality for both bases by tracking the quantities $\|\mathbf{I}_k - \mathbf{V}_k^H \mathbf{V}_k\|_F$ and $\|\mathbf{I}_k - (\mathbf{V}'_k)^H \mathbf{V}'_k\|_F$. Second, we quantified the numerical drift between the two bases by computing the Frobenius norm of their difference, $\|\mathbf{V}_k - \mathbf{V}'_k\|_F$. Third, we evaluated the effect on the final solution by computing the norm of the difference between the two computed vectors, $\|\mathbf{x}_k - \mathbf{x}'_k\|_2$, where $\mathbf{x}_k = \mathbf{V}_k \mathbf{y}_k$ and $\mathbf{x}'_k = \mathbf{V}'_k \mathbf{y}_k$. We hypothesized that both bases would exhibit a similar rate of orthogonality loss. The basis deviation $\|\mathbf{V}_k - \mathbf{V}'_k\|_F$ was expected to be small but non-zero, growing with $k$ due to the accumulation of floating-point errors. Critically, the final solution deviation $\|\mathbf{x}_k - \mathbf{x}'_k\|_2$ was expected to remain close to machine precision, which would demonstrate that the numerical drift in the re-generated basis does not catastrophically degrade the accuracy of the final solution.
