...
In this section, we present a series of numerical experiments designed to validate the theoretical analysis presented in \ref{sec:complexity} and to assess the practical performance of the two-pass Lanczos algorithm. Our investigation is structured into three distinct experiments. The first (Section \ref{sec:exp1}) validates the memory-versus-computation trade-off as a function of iteration count. The second (Section \ref{sec:exp_scalability}) assesses the scalability of both algorithms with respect to the problem dimension. The third (Section \ref{sec:exp_stability}) provides an analysis of numerical stability and accuracy across several challenging problem scenarios.
...
While the KKT systems provide realistic, large-scale test cases for performance, a different methodology is required for the numerical stability and accuracy analysis of general matrix functions. This is because computing an exact ground-truth solution $f(\mathbf{A})\mathbf{b}$ for a large, sparse matrix is a difficult problem in its own right \cite{higham2008functions}. To establish an unambiguous reference, we therefore employ synthetic diagonal matrices, for which $f(\mathbf{A})$ can be computed analytically to machine precision. While this is a standard validation technique, it is important to acknowledge its potential limitations. For functions like the exponential applied to a matrix with a wide spectrum, the resulting solution vector can have components that span many orders of magnitude. This can cause the overall error to be dominated by a few components, potentially distorting the analysis of the polynomial approximation's quality across the entire spectrum. This potential distortion, however, does not invalidate the primary objective of this experiment, which is to assess the numerical equivalence of the one-pass and two-pass methods. Since both algorithms are algebraically identical, they are affected by this spectral property in the same manner, ensuring that a direct comparison of their outputs remains a valid test of the regeneration process's stability. This methodology is detailed further in Section \ref{sec:exp_stability}.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\linewidth]{../results/images/dense-tradeoff.pdf}
    \caption{Wall-clock time vs. iteration count for a dense matrix ($n=10000$)}
    \label{fig:dense_tradeoff_time}
\end{figure}

The results, presented in Figure \ref{fig:dense_tradeoff_time}, align perfectly with this hypothesis. The execution time for both methods scales linearly with the number of iterations, but the slope for the two-pass algorithm is almost exactly double that of the standard method. For example, at $k=1000$ iterations, the standard method takes approximately 70 seconds, while the two-pass method takes approximately 137 seconds.
...
\subsubsection{Results}
We fixed the number of iterations at $k=500$ and analyzed the performance of both algorithms as a function of the problem dimension, $n$. Figures \ref{fig:scalability_memory} and \ref{fig:scalability_time} plot the peak memory usage and wall-clock time against $n$.

\begin{figure}[h!]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../results/images/scalability_k500_rho3_memory.pdf}
        \caption{Peak memory usage vs. problem dimension $n$ for a fixed number of iterations ($k=500$), illustrating the significant $O(nk)$ cost of the standard method. Note that the y-axis is on a logarithmic scale.}
        \label{fig:scalability_memory}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../results/images/scalability_k500_rho3_time.pdf}
...
The memory consumption profiles in Figure \ref{fig:scalability_memory} show a stark contrast between the two methods, confirming our hypothesis. The memory usage of both methods grows linearly\footnote{Note that the plot uses a logarithmic scale for the y-axis to better visualize the differences.} with $n$, but the slope of this growth differs significantly. The two-pass method exhibits a gentle slope, corresponding to the linear growth of the base memory requirement, $M_{\text{base}}(n)$. The standard method, in contrast, shows a much steeper slope, reflecting the combined cost of the base memory and the algorithm-specific term for storing the basis, $M_{\text{std}}(n,k) = M_{\text{base}}(n) + M_{\text{alg, std}}(n, k)$.

In Table \ref{tab:scalability_data}, we provide the raw data underlying the memory plot. The memory usage of the two-pass method serves as a stable empirical baseline for $M_{\text{base}}(n)$. The final column, representing the difference in memory consumption, isolates the cost of storing the basis matrix $\mathbf{V}_k$. A linear regression performed on this column's data yields a growth rate of approximately $3.82$ MB per 1000 units of increase in $n$, which corresponds to 4008 bytes per unit $n$. This observed rate aligns remarkably well with the theoretical rate predicted by our model, which is $k \cdot s_d = 500 \cdot 8 = 4000$ bytes per unit $n$.
...
\begin{table}[ht!]
    ...
    \begin{tabular}{rrrr}
        ...
        \hline
        50,365                                       & 270.4                                      & 141.8                                      & 128.5                                        \\
        100,516                                      & 467.4                                      & 147.2                                      & 320.2                                        \\
        150,632                                      & 666.3                                      & 155.6                                      & 510.6                                        \\
...
The wall-clock time for both algorithms, shown in Figure \ref{fig:scalability_time}, scales linearly with the problem dimension $n$. This is consistent with the theoretical cost, which is dominated by $k$ iterations of operations (such as sparse matrix-vector products) whose complexity is linear in $n$. With $k$ held constant, the total time complexity is $O(nk)$, resulting in the observed linear scaling. For smaller problem dimensions ($n < 150,000$), the two algorithms exhibit nearly identical performance. This aligns with the \emph{equilibrium} regime identified in Section \ref{sec:exp1}, where the cache efficiency benefits of the two-pass method compensate for its additional computational work. As $n$ grows larger, the cost of matrix-vector products becomes more pronounced, and the performance of the two-pass method begins to diverge, becoming progressively slower than the standard method. This confirms that for a sufficiently large number of iterations, the computational cost eventually becomes the dominant factor over memory access patterns.

\subsection{Numerical Accuracy and Stability Analysis}
\label{sec:exp_stability}
In our final experiment, we investigate the numerical accuracy and stability of the two-pass Lanczos method. The primary objective is to verify that the solution computed via basis regeneration is numerically equivalent to the solution from the standard one-pass algorithm. This analysis addresses two fundamental questions: what is the absolute accuracy of the Lanczos approximation for a given problem, and does the two-pass method maintain this accuracy, or does the process of regenerating the basis introduce a numerical degradation?

In order to analyze the numerical accuracy, we compare against a known ground-truth solution, $\mathbf{x}_{\text{true}} = f(\mathbf{A})\mathbf{b}$. For the simple case of linear systems, where $f(z) = z^{-1}$, such a ground truth can be easily constructed for any matrix $\mathbf{A}$ via the standard technique $\mathbf{b} := \mathbf{A}\mathbf{x}_{\text{true}}$ \cite{saad2003iterative}. For more general matrix functions, however, computing an accurate reference solution for a large, sparse matrix is a difficult problem in its own right \cite{higham2008functions}. To circumvent this, our stability experiments employ synthetic diagonal matrices. If $\mathbf{A}$ is a diagonal matrix with eigenvalues $\lambda_i$, the exact solution can be computed analytically to machine precision, as the action of the function simplifies to a component-wise product, $(\mathbf{x}_{\text{true}})_i = f(\lambda_i) (\mathbf{b})_i$. This approach allows us to exercise precise control over the spectral properties of the problem and provides an unambiguous reference against which to measure the relative error of the computed solutions $\mathbf{x}_k$ (one-pass) and $\mathbf{x}'_k$ (two-pass).

We also track the relative deviation $\|\mathbf{x}_k - \mathbf{x}'_k\|_2 / \|\mathbf{x}_k\|_2$ to quantify their numerical equivalence. A small but non-zero deviation is expected. This arises because the two algorithms use different computational paths for solution reconstruction: the one-pass method uses a single matrix-vector product ($\mathbf{x}_k = \mathbf{V}_k \mathbf{y}_k$), while the two-pass method performs an incremental vector accumulation ($\mathbf{x}_k = \sum_j (\mathbf{y}_k)_j \mathbf{v}_j$). These different orderings of floating-point operations lead to minor, predictable differences.

Finally, we assess the stability of the basis generation by quantifying the loss of orthogonality via the Frobenius norm $\|I - \mathbf{V}_k^H \mathbf{V}_k\|_F$ \cite{golub2013matrix}. For this metric, we expect the loss of orthogonality to be computationally identical for both the standard and regenerated bases. This is a direct consequence of the deterministic nature of floating-point arithmetic: the second pass re-executes the same sequence of vector operations using the same stored scalar coefficients as the first pass, resulting in a bit-for-bit identical regenerated basis.
...
The results for this scenario, presented in Figure \ref{fig:acc_exp_well}, confirm this hypothesis. The relative error for both methods converges super-linearly, reaching the level of machine precision in fewer than 30 iterations. The lower panel shows that the relative deviation between the two solutions remains stable and of the order of machine epsilon ($10^{-16}$), consistent with the expected minor differences from their distinct reconstruction paths. The stability of the basis itself is shown in Figure \ref{fig:ortho_exp_well}. The loss of orthogonality increases predictably with the number of iterations and, as expected, the profiles for the standard and regenerated bases are computationally identical.
...
Figure \ref{fig:acc_inv_well} presents the results for this case. The convergence is linear and the error decreases monotonically until it stagnates at the level of machine precision. Again, the two methods produce nearly identical error curves, and their relative deviation is negligible. The underlying basis stability, shown in Figure \ref{fig:ortho_inv_well}, mirrors this result. The loss of orthogonality grows steadily but remains numerically identical for both the stored and regenerated bases.
...
Figure \ref{fig:acc_inv_ill} illustrates this scenario. The error plot shows three distinct phases. Initially, the error stagnates as the Krylov subspace fails to capture the problematic near-null space of $\mathbf{A}$. Subsequently, as the Lanczos process begins to approximate the near-zero eigenvalue, the projected matrix $\mathbf{T}_k$ becomes severely ill-conditioned. This induces a period of erratic, non-monotonic error behaviour, as the inversion of $\mathbf{T}_k$ becomes numerically unstable \cite{golub2013matrix}. Critically, this instability affects both methods identically. Finally, as $k$ becomes sufficiently large, the finite-termination property of the Lanczos method for linear systems comes into effect \cite{saad2003iterative}. The Krylov subspace becomes rich enough to construct a highly accurate polynomial approximant for $z^{-1}$, and the error converges abruptly. Throughout this entire process, the solutions from the one-pass and two-pass methods remain numerically close.  The basis analysis in Figure \ref{fig:ortho_inv_ill} confirms this finding. The severe loss of orthogonality observed coincides with the period of erratic error behavior, affirming that the instability is an intrinsic property of the Lanczos method applied to a nearly singular problem, which the two-pass algorithm correctly reproduces.


\section{Conclusion and Further Work}
Our analysis of the two-pass Lanczos algorithm confirms its effectiveness as a memory-efficient alternative to the standard one-pass method. The experiments show that the standard algorithm's memory usage grows linearly with the iteration count $k$, as predicted by its $O(nk)$ complexity. In contrast, the two-pass variant maintains a constant $O(n)$ memory footprint, making it viable for problems where storing the full Lanczos basis is not feasible.

We also found that the expected twofold increase in runtime for the two-pass method often does not appear in practice. This happens because for large problems, memory bandwidth, not arithmetic, becomes the bottleneck. The standard method's final reconstruction step, $\mathbf{x}_k = \mathbf{V}_k \mathbf{y}_k$, accesses a large basis matrix that often does not fit in processor caches, leading to poor performance. The two-pass method, by operating on a small, fixed-size working set of vectors, shows superior cache efficiency that can offset the cost of its extra matrix-vector products. This effect was so pronounced in some of our tests that the two-pass method ran faster than the standard one. Our stability analysis also showed that the basis regeneration process is numerically sound. Both methods produced nearly identical solutions, even for ill-conditioned problems. The instabilities we observed in the nearly singular case were not a flaw of the two-pass method; they were an intrinsic property of the underlying Lanczos projection, which the two-pass algorithm correctly reproduced.

This work opens several paths for further investigation, particularly for solving linear systems. A natural next step is to compare the two-pass method against the other main low-memory strategy: updating the solution $\mathbf{x}_k$ with a short-term recurrence. This approach forms the basis of the Conjugate Gradient method and avoids storing the full basis, but is a potential source of numerical instability \cite{saad2003iterative}. Such an experiment could directly test the trade-off between the two-pass method's higher computational cost and the stability risks of short-term recurrences.

Another practical direction is to study the algorithm's performance with preconditioning. Krylov methods are almost always used with a preconditioner $\mathbf{M}$ in practice, solving a modified system like $\mathbf{M}^{-1}\mathbf{Ax} = \mathbf{M}^{-1}\mathbf{b}$ \cite{saad2003iterative}. Analyzing how the two-pass method's two stages interact with preconditioning would be critical for assessing its value in realistic computational settings.

Finally, the convergence of Lanczos methods is known to depend on the entire eigenvalue distribution, not just the condition number \cite{golub2013matrix}. Testing the algorithms on matrices with clustered eigenvalues would provide a more complete performance picture and verify the expected rapid convergence in such cases.
