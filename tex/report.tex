...
In order to be able to have the exact computation of solution error, we adopt a known-solution methodology. We first define a ground-truth solution vector $x_{\text{true}}$ and then construct the right-hand side vector as $b := A x_{\text{true}}$.
...
\subsection{Experiment 1: Memory and Computation Trade-off} \label{sec:exp1}
...
\subsection{Experiment 4: Analysis of Basis Orthogonality}
The previous experiment established the numerical equivalence of the final solution vectors. We now complete the analysis by investigating the stability of the basis regeneration process itself. This experiment is designed to measure the loss of orthogonality, a well-known phenomenon in finite-precision implementations of the Lanczos process \cite{golub2013matrix}. While the three-term recurrence guarantees orthogonality in exact arithmetic, rounding errors accumulate and cause a gradual degradation of this property with respect to the more distant vectors in the sequence \cite{frommer2008matrix}.

We use the same four experimental scenarios defined in \ref{sec:exp3} to study how orthogonality loss is affected by different spectral properties. We quantify the degradation using the Frobenius norm of the deviation of $\mathbf{V}_k^H \mathbf{V}_k$ from the identity matrix: $\| \mathbf{I} - \mathbf{V}_k^H \mathbf{V}_k \|_F$ \cite{golub2013matrix}. This metric is computed for both the standard basis $\mathbf{V}_k$, stored during the one-pass algorithm, and the regenerated basis $\mathbf{V}'_k$, recomputed during the second pass.

\subsection{Results}
Figures \ref{fig:orthogonality_exp_well}, \ref{fig:orthogonality_inv_well}, \ref{fig:orthogonality_exp_ill}, and \ref{fig:orthogonality_inv_ill} present the results of this analysis. The data confirms that the loss of orthogonality increases with the number of iterations, a behavior consistent with the theory of the Lanczos process in finite-precision arithmetic \cite{saad2003iterative}. The degradation is visibly more severe for the ill-conditioned problems (Figures \ref{fig:orthogonality_exp_ill} and \ref{fig:orthogonality_inv_ill}), a phenomenon linked to the convergence of Ritz values to the eigenvalues of $\mathbf{A}$ \cite{golub2013matrix}.
...
\begin{figure}[ht!]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Orthogonality loss for $f(z) = \exp(z)$ on well-conditioned matrix}
        \label{fig:orthogonality_exp_well}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../results/images/orthogonality_inv_well-conditioned.pdf}
        \caption{Orthogonality loss for $f(z) = z^{-1}$ on well-conditioned matrix}
        \label{fig:orthogonality_inv_well}
    \end{minipage}
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Orthogonality loss for $f(z) = \exp(z)$ on ill-conditioned matrix}
        \label{fig:orthogonality_exp_ill}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../results/images/orthogonality_inv_ill-conditioned.pdf}
        \caption{Orthogonality loss for $f(z) = z^{-1}$ on ill-conditioned matrix}
        \label{fig:orthogonality_inv_ill}
    \end{minipage}
\end{figure}

The severe loss of orthogonality observed in Figures \ref{fig:orthogonality_inv_ill} and \ref{fig:orthogonality_exp_ill} for the ill-conditioned inverse problem coincides with the period of erratic error behavior in the corresponding solution accuracy plot (Figure \ref{fig:inv_ill_cond}). This confirms that the instability in the solution is an intrinsic part of the Lanczos method applied to a nearly singular problem, which is reproduced by the two-pass algorithm.
