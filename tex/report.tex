...
We conducted this experiment using several KKT system instances of fixed dimension $n$. For each instance and iteration count $k$, we executed multiple independent samples to ensure statistical robustness. The peak RSS and total wall-clock time were recorded for each sample. These measurements were then aggregated to compute the median, as a measure of central tendency, and the standard deviation, to quantify the variability of the results.

\subsubsection{Results}
The experimental results are presented in Figures \ref{fig:large_scale_memory} through \ref{fig:small_scale_time}. The plots show the median of multiple independent samples for each configuration, with shaded bands representing $\pm 1$ standard deviation to quantify measurement variability.
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Median peak memory usage for a large-scale problem (500k arcs). Shaded regions denote $\pm 1$ standard deviation.}
        \label{fig:large_scale_memory}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../results/images/tradeoff_arcs500k_rho3_time.pdf}
        \caption{Median wall-clock time for a large-scale problem (500k arcs). Shaded regions denote $\pm 1$ standard deviation.}
        \label{fig:large_scale_time}
    \end{minipage}
\end{figure}

For the large-scale problem instance, the memory consumption profiles in Figure \ref{fig:large_scale_memory} confirm the theoretical complexity models. The standard one-pass algorithm exhibits a linear growth in median memory usage, consistent with its $O(nk)$ complexity. The standard deviation is negligible, reflecting a deterministic allocation profile dominated by the storage of the $n \times k$ basis matrix $\mathbf{V}_k$. The two-pass algorithm maintains a constant median memory footprint, aligning with its $O(n)$ complexity, but displays a measurable, non-zero standard deviation. This variability is not algorithmic but systemic, arising from the dynamic interaction between the program's fine-grained memory requests and the operating system's memory manager. The analysis of wall-clock time for the large-scale instance, presented in Figure \ref{fig:large_scale_time}, reveals a critical insight into the performance of these algorithms on modern hardware. The median execution times of both methods are nearly identical, and their uncertainty bands exhibit substantial overlap across the entire range of $k$. This indicates that there is no statistically significant performance difference between them, a result that seemingly contradicts the theoretical model predicting a factor-of-two slowdown for the two-pass method due to doubled matrix-vector products.

This convergence in performance is a direct consequence of the system operating in a memory bandwidth-bound regime. For a problem of this scale, the memory footprint of the core data structures far exceeds the capacity of the CPU caches. As a result, the execution time of nearly all vector operations, both sparse and dense, is dominated by the latency of data transfer from main memory, rather than the cost of floating-point arithmetic. The final solution reconstruction in the standard method, $\mathbf{x}_k = \mathbf{V}_k \mathbf{y}_k$, requires streaming the entire $n \times k$ basis from DRAM, a costly operation. While the two-pass method performs twice as many sparse matrix-vector products, an operation also bound by memory access, it avoids the large, monolithic memory transfer of the final reconstruction. In this regime, the high, common cost of memory access for both algorithms becomes the primary bottleneck, diminishing the relative impact of the additional arithmetic operations. The theoretical computational penalty of the two-pass method is thus effectively masked by the architectural limitations of memory bandwidth.
...
\begin{figure}[ht!]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Median peak memory usage for a medium-scale problem (50k arcs). The crossover point highlights the initial overhead of the two-pass method.}
        \label{fig:medium_scale_memory}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../results/images/tradeoff_arcs50k_rho3_time.pdf}
        \caption{Median wall-clock time for a medium-scale problem (50k arcs). The two-pass method is consistently faster due to better cache utilization.}
        \label{fig:medium_scale_time}
    \end{minipage}
\end{figure}

In the medium-scale instance, shown in Figure \ref{fig:medium_scale_memory}, the memory profiles remain consistent with the theoretical models. A crossover point is visible, where for a low number of iterations ($k < 100$), the fixed memory overhead of the two-pass method (for storing $\mathbf{b}$, $\{\alpha_j, \beta_j\}$, and $\mathbf{y}_k$) exceeds the growing $O(nk)$ cost of the standard method's basis. As $k$ increases, the $O(nk)$ term dominates, and the standard method's memory usage surpasses that of the two-pass variant. Figure \ref{fig:medium_scale_time} reveals a performance inversion. The median execution time of the two-pass method is now consistently lower than that of the standard algorithm, and their uncertainty bands are clearly separated for $k > 400$. This transition to a cache-bound regime occurs because the basis matrix $\mathbf{V}_k$ becomes too large to reside in CPU caches, causing a high cache miss rate during the final solution reconstruction $\mathbf{x}_k = \mathbf{V}_k \mathbf{y}_k$. The two-pass method, with its small, cache-resident working set, avoids this penalty, and the resulting performance gain from higher cache efficiency outweighs the cost of additional arithmetic operations.
...
\begin{figure}[ht!]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Median peak memory usage for a small-scale problem (5k arcs).}
        \label{fig:small_scale_memory}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../results/images/tradeoff_arcs5k_rho3_time.pdf}
        \caption{Median wall-clock time for a small-scale problem (5k arcs). The cache advantage of the two-pass method is maximized.}
        \label{fig:small_scale_time}
    \end{minipage}
\end{figure}

The trends observed in the medium-scale instance are maximized for the small-scale problem, as shown in Figures \ref{fig:small_scale_memory} and \ref{fig:small_scale_time}. The memory usage of the standard method is nearly constant across the tested range of $k$, as the total size of the basis matrix remains small enough to not significantly alter the process's total memory footprint relative to the base cost. The performance advantage of the two-pass method, shown in Figure \ref{fig:small_scale_time}, is pronounced. In this regime, the working set of the two-pass algorithm can be accommodated by the fastest levels of the cache hierarchy, leading to a significant and consistent reduction in wall-clock time compared to the standard method.
