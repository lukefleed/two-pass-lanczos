...
/// * `t_k`: A reference to the tridiagonal matrix view `T_k` generated so far.
///
/// # Returns
...
pub type LanczosCallback<T> = dyn for<'a> FnMut(
    usize,
    MatRef<'a, T>,
    &'a TridiagonalView<'a, <T as ComplexField>::Real>,
) -> bool;

/// A read-only view of a tridiagonal matrix without owning the data.
///
/// This struct provides zero-allocation access to tridiagonal matrix data
/// by referencing existing slices instead of cloning vectors.
#[derive(Debug)]
pub struct TridiagonalView<'a, R: RealField> {
...
/// memory allocations within the iterative loop
///
/// # Arguments
...
/// * `stack`: Memory stack for temporary allocations. Should be sized with `operator.apply_scratch()`.
    ...
    for i in 0..k {
        if let Some(step) = lanczos_iter.next_step(stack) {
            ...
            if let Some(ref mut cb) = callback {
                ...
                let current_t_k_view = TridiagonalView {
                    alphas: &alphas,
                    betas: &betas,
                    steps_taken,
                };

                // Invoke the callback - if it returns false, stop iteration
                if !cb(steps_taken, current_v_k, &current_t_k_view) {
                    break;
                }
            }
...
mod tests {
    ...
    fn test_lanczos_pass_one_produces_correct_dimensions() {
        ...
        let mut mem = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let mut stack = MemStack::new(&mut mem);
        let t_k = lanczos_pass_one(&a.as_ref(), b.as_ref(), k, &mut stack).unwrap();
    ...
    fn test_one_pass_and_pass_one_produce_same_tridiagonal() {
        ...
        let mut mem1 = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let mut stack1 = MemStack::new(&mut mem1);
        let standard_result =
            lanczos_standard(&a.as_ref(), b.as_ref(), k, &mut stack1, None).unwrap();

        let mut mem2 = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let mut stack2 = MemStack::new(&mut mem2);
        let pass_one_result = lanczos_pass_one(&a.as_ref(), b.as_ref(), k, &mut stack2).unwrap();
    ...
    fn test_lanczos_relation_holds() {
        ...
        let mut mem1 = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let mut stack1 = MemStack::new(&mut mem1);
        let result_k = lanczos_standard(&a.as_ref(), b.as_ref(), k, &mut stack1, None).unwrap();
    ...
    fn test_two_pass_reconstruction() {
        ...
        let mut mem2 = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let mut stack2 = MemStack::new(&mut mem2);
        let t_k = lanczos_pass_one(&a.as_ref(), b.as_ref(), k, &mut stack2).unwrap();
        ...
        let mut mem3 = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let mut stack3 = MemStack::new(&mut mem3);
        let x_k_two_pass =
            lanczos_pass_two(&a.as_ref(), b.as_ref(), &t_k, y_k.as_ref(), &mut stack3).unwrap();
    ...
    fn test_breakdown_scenario() {
        ...
        let mut mem2 = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let mut stack2 = MemStack::new(&mut mem2);
        let t_k_pass_one = lanczos_pass_one(&a.as_ref(), b.as_ref(), k, &mut stack2).unwrap();
    ...
    fn test_zero_vector_input_returns_error() {
        ...
        let mut mem1 = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let mut stack1 = MemStack::new(&mut mem1);
        assert!(lanczos_standard(&a, b.as_ref(), 2, &mut stack1, None).is_err());

        let mut mem2 = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let mut stack2 = MemStack::new(&mut mem2);
        assert!(lanczos_pass_one(&a, b.as_ref(), 2, &mut stack2).is_err());
        ...
        let mut mem3 = MemBuffer::new(a.apply_scratch(1, Par::Seq));
        let mut stack3 = MemStack::new(&mut mem3);
        assert!(lanczos_pass_two(&a, b.as_ref(), &t_k, y_k.as_ref(), &mut stack3).is_err());
    }
}
